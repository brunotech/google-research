...
"""Experiments on 3 Sized Bags Large Margin."""
import random
    ...
    for k in range(K):
      ...
      while i < N + 100:
        ...
        if i >= n_nonmon:  # monochromatic bag
          ...
          if (hardsign(np.dot(x2, r)) == hardsign(np.dot(x1, r))) & (hardsign(
              np.dot(x3, r)) == hardsign(np.dot(x1, r))):
            x2 = (-1) * x2
...
## Test part
      ...
      if D == 10 and N == 50 and k == 0:
        df.to_csv("large_margin_3_sized_results.csv", index=False)
      else:
        df.to_csv(
            "large_margin_3_sized_results.csv",
            index=False,
            mode="a",
            header=False)
