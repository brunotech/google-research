...
class MultiHeadLayer(Layer):
  ...
  def build(self, input_shape):
    if len(input_shape) != 3:
      raise ValueError(
          'Unexpected dimensions %d, expect to be 3 dimensions' %
          (len(input_shape)))
...
class AutoInt:
  ...
  def __init__(self,
               ...
               embed_size=8,
               nb_multihead=3):
    self.pre_process(my_data)
    self.embed_size = embed_size
    self.nb_multihead = nb_multihead
    self.nb_class = 2

  def pre_process(self, my_data):
    cols = my_data.columns.values
    self.sparse_feats = [f for f in cols if '_id' not in f]
  ...
  def embedding_layers(self):
    """Embedding layers for sparse fratures."""
  ...
  def construct(self, cluster_per_class=250, model_type='selfclr_llp'):
    """Construct auto-int architecture."""
    self.input_layers()
    self.embedding_layers()
    embed_layer = Concatenate(axis=1)(self.sparse_kd_embed)
    x = embed_layer
